...
class TextMatchingForBert(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.is_train:
            ...
            query_output = pooled_output[0::3, ]
            pos_doc_output = pooled_output[1::3, ]
            neg_doc_output = pooled_output[2::3, ]

            query_output_norm = query_output / query_output.norm(dim=1)[:, None]
            pos_output_norm = pos_doc_output / pos_doc_output.norm(dim=1)[:, None]
            neg_output_norm = neg_doc_output / neg_doc_output.norm(dim=1)[:, None]

            pos_scores = torch.matmul(query_output_norm, pos_output_norm.transpose(0, 1)) * 20.0
            neg_scores = torch.matmul(query_output_norm, neg_output_norm.transpose(0, 1)) * 20.0

            probability = torch.exp(pos_scores) / (torch.exp(pos_scores) + torch.exp(neg_scores))
            loss = torch.sum(-torch.log(torch.diagonal(probability)))
